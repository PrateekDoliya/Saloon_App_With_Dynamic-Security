...
public interface PermissionDtoRepository extends JpaRepository<PermissionDto, Integer> {
	...
	@Query(nativeQuery = true, value="select p.permission_id,p.permission from maikhane_employee_service_saloon_app.permission p where p.permission_id=:permissionId")
